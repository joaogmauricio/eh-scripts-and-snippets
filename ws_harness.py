...
def FuzzWebSocket(fuzz_value):
    print fuzz_value
    ws.send(ws_message.replace("[FUZZ]", str(fuzz_value[0]).replace("\"","").replace("\\","")))
    result = ws.recv()
    return result
...
ws = create_connection(args.url,sslopt={"cert_reqs": ssl.CERT_NONE},header={})
#ws = create_connection(args.url,sslopt={"cert_reqs": ssl.CERT_NONE},header={},http_proxy_host="localhost", http_proxy_port=8080)
ws.send(ws_message.replace("[FUZZ]", "mock"))
ws.recv()
ws.recv()
ws.recv()
